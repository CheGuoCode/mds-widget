    ...
    beforeAppearMaker() {
        globalEvent.addEventListener('viewWillAppear', (options) => {
            if (options.type === 'open' || options.type === 'refresh') {
                ...
                storage.getData('router.backParams', ({ status, code , errorMsg, data }) => {
                    const result = status === 0 || code === 0 ? JSON.parse(data) : ''
...
Mixins.install = (Vue, options) => {
    Vue.mixin({
        beforeCreate () {
            if (!this.$options.mds) return
            const mdsEvents = this.$options.mds
            const mdsEventsMap = Object.keys(this.$options.mds)
            new EventsMaker(mdsEventsMap)
            mdsEventsMap.map(event => {
                if (!GLOBAL_EVENTS[event]) GLOBAL_EVENTS[event] = []
                GLOBAL_EVENTS[event].push(mdsEvents[event].bind(this))
            })
        }
    })
}
